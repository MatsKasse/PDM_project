...
from mpc.static_obstacle_to_circles import static_obstacles_to_circles, filter_circles_near_robot_capped, draw_circle_pybullet
...
def run_albert(n_steps=1000, render=False, path_type="straight", path_length=3.0, return_metrics=False):
    robots = [
...
# ================= A_STAR ========================================================================
    ...
    #Calculate path length
        ...
        world_min, world_max = get_world_bounds()
        x_min, y_min, _ = world_min
        x_max, y_max, _ = world_max

        #Generate gridmap and inflated gridmap
        grid, inflated_grid = generate_gridmap(x_min, x_max, y_min, y_max, resolution=resolution, robot_radius=robot_radius)

        #Convert start and goal to grid coordinates
        sx_g, sy_g = world_to_grid(sx, sy, x_min, y_min, resolution)    
        gx_g, gy_g = world_to_grid(gx, gy, x_min, y_min, resolution)
        ...
        rx_w, ry_w = zip(*[grid_to_world(x, y, x_min, y_min, resolution) for x, y in zip(rx_g, ry_g)])
...
if __name__ == "__main__":
    ...
    with warnings.catch_warnings():
        ...
        for i in range(n_runs):  # run multiple trials
            history, metrics = run_albert(n_steps=3000, render=True, return_metrics=True)
